  ...
  useEffect(() => {
    ...
    class AutoDriver {
      constructor(mouse, manager, opts) {
        ...
        this.mouse.coords.set(this.startX, 0);
        this.mouse.coords_old.set(this.startX, 0);
      }
      ...
      update() {
        ...
        this.currentX += this.speed * dtSec;

        const elapsed = (now - this.startTime) / 1000;
        const y = Math.sin(elapsed * 1.15) * 0.1;

        if (this.currentX >= this.endX) {
          this.currentX = this.startX;

          this.mouse.coords.set(this.currentX, y);
          this.mouse.coords_old.set(this.currentX, y);
          this.mouse.diff.set(0, 0);

          return;
        }

        this.mouse.setNormalized(this.currentX, y);
      }
    }
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    float lenv = clamp(length(vel) * 5.0, 0.0, 1.0);
...
`;
    ...
    void main(){
      float wave1 = sin(uv.x * 10.0 - time * 1.1) * 0.045;
      float wave2 = sin(uv.x * 18.0 + time * 0.65) * 0.018;

      float centerY = 0.5 + wave1 + wave2;

      float distanceFromFlow = abs(uv.y - centerY);

float thickness1 =
  sin(uv.x * 7.0 + time * 0.55) * 0.035;

float thickness2 =
  sin(uv.x * 15.0 - time * 0.8) * 0.022;

float thickness3 =
  sin(uv.x * 27.0 + time * 0.35) * 0.012;
...
float band = 1.0 - smoothstep(
  ...
  distanceFromFlow
      ...
      float flowA =
  sin(uv.x * 7.0 + time * 0.8);

float flowB =
  sin(uv.x * 13.0 - time * 0.55);

float flowC =
  sin(uv.x * 23.0 + time * 0.32);
...
float forceY =
  ...
  band *
  (
    cos(uv.x * 9.0 - time * 0.7) * 0.16 +
    sin(uv.x * 17.0 + time * 0.45) * 0.08
  );
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps) {
        ...
        const flowGeometry = new THREE.PlaneGeometry(2, 2);

        const flowMaterial = new THREE.RawShaderMaterial({
          vertexShader: face_vert,
        ...
        this.flow = new THREE.Mesh(flowGeometry, flowMaterial);

        this.scene.add(this.flow);
      }

      update(props) {
        const uniforms = this.flow.material.uniforms;

        uniforms.time.value = Common.time;

        uniforms.strength.value = props.mouse_force * 0.00075;
